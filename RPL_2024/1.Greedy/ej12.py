...
def asignar_mafias(pedidos):
    if(len(pedidos) == 0):
        return []
    pedidosOrdenados = sorted(pedidos, key=lambda x: x[1])
    pedidosConfirmados = []
    pedidosConfirmados.append(pedidosOrdenados[0])
    for i in range(1,len(pedidosOrdenados)):
        finalRangoConfirmado = pedidosConfirmados[-1][1]
        inicioRangoPedido = pedidosOrdenados[i][0]
        if(inicioRangoPedido >= finalRangoConfirmado):
            pedidosConfirmados.append(pedidosOrdenados[i])
        
    return pedidosConfirmados

resul = asignar_mafias([(5, 10), (12, 15)])
print(resul)